var gatheringCodeVM = new Vue({
    ...
    methods: {
        ...
        addOrUpdateGatheringCode: function () {
            ...
            // for (var i = 0; i < that.gatheringChannelDictItems.length; i++) {
            //     var item = that.gatheringChannelDictItems[i];
            //     if (!editGatheringCode.gatheringUid && item.channelId == editGatheringCode.gatheringChannelId && item.channelCode == "alipay" && !that.unfixedGatheringCodeReceiveOrder) {
            //         layer.alert('支付宝固码类型,必需填写支付宝帐号,如果不知道如何获取自己的支付宝帐号,请百度或者联系管理员', {
            //             title: '提示',
            //             icon: 7,
            //             time: 3000
            //         });
