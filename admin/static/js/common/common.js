...
function initGlobal() {
	...
	var receiveOrderSetting = sessionStorage.getItem('receiveOrderSetting');
	if (receiveOrderSetting != null) {
		GLOBAL.receiveOrderSetting = JSON.parse(receiveOrderSetting);
	} else {
		loadReceiveOrderSetting();
	}
}
...
function loadReceiveOrderSetting() {
	var that = this;
	Vue.http.get('/masterControl/getReceiveOrderSetting').then(function(res) {
		GLOBAL.receiveOrderSetting = res.body.data;
		sessionStorage.setItem('receiveOrderSetting', JSON.stringify(GLOBAL.receiveOrderSetting));
	});
}
